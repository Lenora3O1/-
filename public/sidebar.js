(function(){
  ...
  if(sidebar){
    ...
    toggle?.addEventListener('click',()=>{if(window.innerWidth<=900)document.body.classList.toggle('sidebar-open');else setCollapsed(!document.body.classList.contains('sidebar-collapsed'));});
    backdrop?.addEventListener('click',()=>document.body.classList.remove('sidebar-open'));
    sidebar.querySelectorAll('.nav-tabs button').forEach(btn=>btn.addEventListener('click',()=>{if(window.innerWidth<=900)document.body.classList.remove('sidebar-open');}));
  }
  ...
  function sync(){const logged=main?getComputedStyle(main).display!=='none':(login?getComputedStyle(login).display==='none':true);document.body.classList.toggle('is-authenticated',logged);}
